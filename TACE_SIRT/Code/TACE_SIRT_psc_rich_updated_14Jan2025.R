...
levels(flexspline_model1$data$m$Cirrhosis)
str(flexspline_model1$data$m$Cirrhosis)
str(flexspline_model1$data$m$AFP)
str(flexspline_model1$data$Albumin)
str(flexspline_model1$data$m$Lesion1)

str(sirt_data1)
str(flexspline_model1$data)
...
####As the line of code   #####  p+ geom_line(data=s_data, aes(time,S),col=4,lwd=1.5)  ### does not work or gives a null, try another approach
library(ggplot2)
# Check if psc_tace2 contains survival curves or predictions
s2 <- surv_fpm(psc_tace2$DC_clean, beta=-0.245)
s_data <- data.frame("time" = s2$time, "S" = s2$S)
# Construct ggplot manually
p <- ggplot() +
  geom_line(data = s_data, aes(x = time, y = S), col = 4, lwd = 1.5) +
  labs(x = "Time", y = "Survival Probability") +
  theme_minimal()

print(p)  # Ensure it's displayed
